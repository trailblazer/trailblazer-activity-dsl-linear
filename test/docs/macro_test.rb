...
class DocsMacroTest < Minitest::Spec
  it "what" do
    ...
    ctx, _, signal = A::Create.({params: {user: "Adam"}}, {}, {})
  ...
  it "what" do
      ...
      module MyMacro
        def self.FindModel(model_class)
          ...
          task = ->(ctx, flow_options, _) do
            ...
            return ctx, flow_options, return_signal
          end
    ...
    # ctx, _, signal = Trailblazer::Developer.wtf?(B::Create, [{params: {}}])
    # _(ctx.inspect).must_equal %{{:params=>{}, :model=>nil}}
    # _(signal.inspect).must_equal %{#<Trailblazer::Activity::End semantic=:not_found>}

=begin
#:output-result
ctx, _, signal = Trailblazer::Developer.wtf?(User::Create, [{params: {id: nil}}])
